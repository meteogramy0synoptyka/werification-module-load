...
Poland = bounds(ymin=135, ymax=310, ylen=310-135,
                xmin=120, xmax=285, xlen=285-120)
...
with open(os.path.join(directory, "um_settings", name1, lat_name), 'rb') as f:
    lat = load(f)

with open(os.path.join(directory, "um_settings", name1, lon_name), 'rb') as f:
    lon = load(f)
...
def make_mask(my_bounds):
    ...
    mask = np.array(my_bounds.xlen*my_bounds.ylen *
                    [False]).reshape(my_bounds.xlen, my_bounds.ylen)
    for row in rows:
        for col in cols:
